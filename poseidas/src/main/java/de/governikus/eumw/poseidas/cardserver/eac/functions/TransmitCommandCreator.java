...
public interface TransmitCommandCreator<S extends FunctionParameter> extends APDUCountRestricted
{
  ...
  /**
   * Creates list of transmit commands for parameter (one or more plain commands, accepting all response codes).
   *
   ...
   *
   * @param parameter parameter
   * @param acceptedResponseList optional list of only acceptable responses, batch execution is to be stopped on first
   *          not acceptable response code
   * @return transmit commands
   * @throws Exception when creating fails, exception according internal creation steps
   */
  public List<InputAPDUInfoType> create(S parameter, List<ResponseAPDU> acceptedResponseList) throws Exception;
