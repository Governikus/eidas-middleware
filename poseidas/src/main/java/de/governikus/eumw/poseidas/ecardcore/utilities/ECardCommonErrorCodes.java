...
public enum ECardCommonErrorCodes implements ECardErrorCode
{
  ...
  FILE_IS_EMPTY("file.is_empty", startCode() + 1, ResultMinor.COMMON_PARAMETER_ERROR),
  /** if the file object is a directory and not a regular file */
  @ECardErrorCodeParam(params = {"file"})
  FILE_IS_A_DIR("file.is_a_dir", startCode() + 2, ResultMinor.COMMON_PARAMETER_ERROR),
  /** if the file could not be read */
  @ECardErrorCodeParam(params = {"file"})
  FILE_CANT_READ("file.cant_read", startCode() + 3, ResultMinor.COMMON_PARAMETER_ERROR),
  /** if the file could not be write */
  @ECardErrorCodeParam(params = {"file"})
  FILE_CANT_WRITE("file.cant_write", startCode() + 4),
  /** if the file object is null */
  FILE_IS_NULL("file.is_null", startCode() + 5, ResultMinor.COMMON_PARAMETER_ERROR),
  /** if the file could not be found on the volume */
  @ECardErrorCodeParam(params = {"file"})
  FILE_NOT_FOUND("file.not_found", startCode() + 6, ResultMinor.COMMON_PARAMETER_ERROR),
  /** if the file could not be deleted */
  @ECardErrorCodeParam(params = {"file"})
  FILE_CANT_DELETE("file.cant_delete", startCode() + 7),
  /** if the file can't be parsed, it is invalid */
  @ECardErrorCodeParam(params = {"file"})
  FILE_INVALID("file.invalid", startCode() + 8, ResultMinor.COMMON_PARAMETER_ERROR),
  /** if the security manager denied the file access */
  @ECardErrorCodeParam(params = {"file"})
  FILE_PERMISSION_DENIED("file.permission_denied", startCode() + 9, ResultMinor.COMMON_PARAMETER_ERROR),
  ...
  COMMON_PARAMETERMISSING("common.requiredparametersmissing",
                          startCode() + 15,
                          ResultMinor.COMMON_PARAMETER_ERROR),
  /**
   * If a parameter has a wrong value or a wrong format, so that the request can not be fulfilled.
   */
  COMMON_WRONGPARAMETERS("common.wrongparameter", startCode() + 16, ResultMinor.COMMON_PARAMETER_ERROR),
