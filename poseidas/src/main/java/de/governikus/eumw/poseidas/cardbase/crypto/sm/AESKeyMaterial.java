...
/**
 * Implementation of key material based on AES keys used for data field encryption and CMAC calculation as it is used
 * for nPA.
 *
...
public class AESKeyMaterial
{
   ...
   * Constructor.
   *
   * @param aesEncKey AES key for encryption and decryption of data field, <code>null</code> or keys other than AES not
   *          permitted, encryption key and mac calculation key must be of same strength
   * @param aesMacKey AES key for CMAC calculation, <code>null</code> or keys other than AES not permitted, encryption
   *          key and mac calculation key must be of same strength
   * @param ivParameterSpecSSC IvParameterSpec as SSC related parameter spec, <code>null</code> not permitted permitted
   * @throws IllegalArgumentException if keys or IvParameterSpec not permitted, also if keys of different strength used
   */
  public AESKeyMaterial(SecretKey aesEncKey, SecretKey aesMacKey, AESEncSSCIvParameterSpec ivParameterSpecSSC)
  {
    ...
    {
      throw new IllegalArgumentException("only IvParameterSpec with " + CipherUtil.AES_IV_LENGTH + " bytes permitted");
    }
