...
public interface HSMServiceFactory extends Service
{
  ...
  /**
   * Gets interface to requested HSM and sets this as the default which will be returned by subsequent calls to
   * {@link #getHSMService()}.
   *
   * @param hsmType type of HSM, currently possible values are {@link HSMService#NO_HSM}, {@link HSMService#PKCS11_HSM}
  ...
  /**
   * Gets interface to default HSM (default is the HSM which has been requested by {@link #getHSMService(int)} or
   * {@link #getHSMService(Class)} before; if such a call has not been made, using no HSM is default).
   *
