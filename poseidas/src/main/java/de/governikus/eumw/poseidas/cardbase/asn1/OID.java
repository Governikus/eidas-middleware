...
public class OID extends AbstractASN1Encoder implements ASN1Encoder
{
   ...
   * Constructor.
   *
   * @param oidString String representation, <code>null</code> or empty String not permitted, String containing only
   *          decimal digits and OID separator permitted
   * @see #toBytes(String)
  ...
  public OID(byte[] bytes) throws IOException
  {
    ...
    if (super.getTag().toByteArray().length != 1 || super.getTag().toByteArray()[0] != ASN1Constants.UNIVERSAL_TAG_OID)
    {
