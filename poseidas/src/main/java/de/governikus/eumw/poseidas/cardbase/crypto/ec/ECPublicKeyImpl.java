...
/**
 * Implementation of {@link ECPublicKey} as substitution of SUN implementation supporting only SUN known curves.
 *
...
public final class ECPublicKeyImpl implements ECPublicKey
{
  ...
  /**
   * Public point W. Note: we have inherited {@link Serializable} through {@link ECPublicKey} but would run into errors
   * serializing {@link ECPoint} and {@link ECParameterSpec} so we declare the fields transient as we do not need to
   * serialize instances of this class.
   */
  private final transient ECPoint w;

  /**
   * Domain parameters. Note: we have inherited {@link Serializable} through {@link ECPublicKey} but would run into
   * errors serializing {@link ECPoint} and {@link ECParameterSpec} so we declare the fields transient as we do not need
  ...
  public byte[] getEncoded()
  {
    ...
    ASN1 generator = new ASN1(0x04,
                              ByteUtil.combine(new byte[][]{new byte[]{0x04},
    ...
    ASN1 innerDomParamSeq = new ASN1(0x30,
                                     ByteUtil.combine(new byte[][]{version.getEncoded(), primeSeq.getEncoded(),
                                                                   curveSeq.getEncoded(), generator.getEncoded(),
                                                                   order.getEncoded(), cofactor.getEncoded()}));
    ...
    byte[] ptBytes = ByteUtil.combine(new byte[][]{new byte[]{0x00, 0x04},
                                                   ByteUtil.trimByteArray(w.getAffineX().toByteArray(), trimLen),
                                                   ByteUtil.trimByteArray(w.getAffineY().toByteArray(), trimLen)});
