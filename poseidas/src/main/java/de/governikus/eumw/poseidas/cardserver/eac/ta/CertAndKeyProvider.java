...
public interface CertAndKeyProvider
{
   ...
   * @return List of certificates, <code>null</code> if not found, list expected to contain certificates in order from
   *         root at index 0 to child at highest index, list must not contain the root certificate itself and must not
   *         contain the terminal certificate
   * @throws IllegalArgumentException if any parameter <code>null</code>
   */
  public abstract List<byte[]> getCertChain(String rootHolder, String termHolder) throws IOException;
