...
class BlackListTimerTest
{
  ...
  {
    blackListTimer.run();
    Mockito.verify(permissionDataHandling, Mockito.times(1)).renewBlackList(true, true);
  }

  @Test
  void testBlackListTriggerWithInitialDelay()
  {
    Trigger blackListTrigger = blackListTimer.getBlackListTrigger(new ArrayList<>());
