...
class StatusControllerTest extends WebAdminTestBase
{
  ...
  void testWhenCvcTlsCheckEmptyThenStatusPageWithMessageAndNoValues() throws Exception
  {
    ...
    HtmlPage loginPage = getWebClient().getPage(getRequestUrl("/status"));
  ...
  private void assertFirstServiceProvider(DomElement domElement, Date expectedDate)
  {
    ...
    Assertions.assertEquals(4, cvcSpanValues.getLength());
    ...
    Assertions.assertEquals(expectedDate.toInstant()
                                        ...
                                        .minusDays(1)
                                        .toString(),
                            cvcSpanValues.item(1).getTextContent());
    Assertions.assertEquals(OK, cvcSpanValues.item(2).getTextContent());
    Assertions.assertEquals(OK, cvcSpanValues.item(3).getTextContent());
    // RSC
  ...
  {
    try (
      InputStream requestSignerCertificateAsInputStream = StatusControllerTest.class.getResourceAsStream("/requestSigner.cer");)
    {
