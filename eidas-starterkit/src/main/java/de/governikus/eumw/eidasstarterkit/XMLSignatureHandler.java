...
/**
 * <p>
 * Helper for adding a signature to a SignableXMLObject. Please note that this adds everything except the signature
 * value itself. Make sure to add that after marshalling the XML object. Furthermore, this class can be used for
 * validating the signatures.
 * </p>
 * <p>
 * WARNING: the underlying XML security implementation has a bug which disables signing after verifying with same
 * algorithm in same thread. This class does not contain a work-around for that problem because it does not arise in
 * many scenarios. If necessary, open a new thread for signing!
 * </p>
 * <p>
 * </p>
 *
...
final class XMLSignatureHandler
{
  ...
  /**
   * Add a signature to a SignableXMLObject. Please note that this adds everything except the signature value itself.
   * Make sure to add that after marshalling the XML object.
   *
